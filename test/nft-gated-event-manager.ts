...
describe("NFTGatedEventManager", function () {
  ...
  describe("registerForEvent", function () {
    it("should revert if event to register for is not currently active", async function () {
      const { nftGatedEventManager } = await loadFixture(deployNFTGatedManager);
      expect(nftGatedEventManager.registerForEvent(1)).to.be.revertedWith(
        "Event is not active."
      );
    });

    it("should revert if event time has passed", async function () {
      const { nftGatedEventManager, nft } = await loadFixture(
        deployNFTGatedManager
      );
      const eventName = "Lagos Day Party";
      const eventDate = (await time.latest()) + 3600;
      ...
      expect(nftGatedEventManager.registerForEvent(0)).to.be.revertedWith(
    ...
    it("should revert if maxCapacity for event is reached", async function () {
      const { nftGatedEventManager, addr1, addr2, nft } = await loadFixture(
        deployNFTGatedManager
      );
      const eventName = "Lagos Day Party";
      const eventDate = (await time.latest()) + 3600;
      ...
      await nft
        .connect(addr1)
        .mintToken({ value: ethers.parseEther("0.0001") });
      await nft
        .connect(addr2)
        .mintToken({ value: ethers.parseEther("0.0001") });
      ...
      expect(
        nftGatedEventManager.connect(addr2).registerForEvent(0)
      ).to.be.revertedWith("Event is fully booked.");
    });
    ...
    it("should register users for event and verify registration", async function () {
      const { nftGatedEventManager, nft, owner, addr1, addr2 } =
        await loadFixture(deployNFTGatedManager);
      const eventName = "Lagos Day Party";
      const eventDate = (await time.latest()) + 3600;
      const nftRequired = await nft.getAddress();
      const maxCapacity = 5;
      ...
      expect(await nftGatedEventManager.isUserRegistered(0, owner)).to.equal(
        true
      );
      expect(await nftGatedEventManager.isUserRegistered(0, addr1)).to.equal(
        true
      );
      expect(await nftGatedEventManager.isUserRegistered(0, addr2)).to.equal(
        true
      );
    });
  });
  ...
  describe("check if user is registered", function () {
    it("should check if a user is registered", async function () {
      const { nftGatedEventManager, nft, owner } = await loadFixture(
        deployNFTGatedManager
      );
      const eventName = "Lagos Day Party";
      const eventDate = (await time.latest()) + 3600;
      ...
      expect(await nftGatedEventManager.registerForEvent(0))
        .to.emit(nftGatedEventManager, "UserRegistered")
        .withArgs(0, owner);
      const userRegistered = await nftGatedEventManager.isUserRegistered(
        0,
        owner
      );
